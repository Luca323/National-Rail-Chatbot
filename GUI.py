...
def send_message(e, txt):
    ...
    userMsg = f"[{currentTime}] You -> {e.get()}"
    txt.insert(END, "\n" + userMsg)
    ...
    if (user == "hello"):
        botMsg = f"[{currentTime}] Bot -> Hi there, how can I help?"

    elif (user == "goodbye"):
        botMsg = f"[{currentTime}] Bot -> Thank you for using this service."

    else:
        botMsg = f"[{currentTime}] Bot -> I don't understand you."
